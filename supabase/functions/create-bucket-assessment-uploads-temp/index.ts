...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'assessment-uploads',
        name: 'assessment-uploads',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["image/*", "application/pdf", "text/csv", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for assessment-uploads" ON storage.objects FOR SELECT USING (bucket_id = 'assessment-uploads');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for assessment-uploads" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'assessment-uploads');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for assessment-uploads" ON storage.objects FOR UPDATE USING (bucket_id = 'assessment-uploads');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for assessment-uploads" ON storage.objects FOR DELETE USING (bucket_id = 'assessment-uploads');`
    ];
